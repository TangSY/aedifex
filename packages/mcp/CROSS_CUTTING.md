...
The concrete backend is now `SqliteSceneStore`, backed by built-in SQLite drivers (`bun:sqlite` for the MCP CLI and `node:sqlite` for the Next.js editor server). It writes to `~/.aedifex/data/aedifex.db` by default and also supports `AEDIFEX_DATA_DIR`, `AEDIFEX_DB_PATH`, and `AEDIFEX_MAX_SCENE_BYTES`.
