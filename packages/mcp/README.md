...
AEDIFEX_DATA_DIR="$HOME/.aedifex/data" bun run dev

# Terminal 2 or an MCP host: run the server
AEDIFEX_DATA_DIR="$HOME/.aedifex/data" bun packages/mcp/dist/bin/aedifex-mcp.js
```
...
```bash
claude mcp add pascal bunx aedifex-mcp
```
...
```bash
codex mcp add pascal --env AEDIFEX_DATA_DIR="$HOME/.aedifex/data" -- bunx aedifex-mcp
```
