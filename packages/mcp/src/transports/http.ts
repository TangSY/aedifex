...
const ALLOWED_HEADERS =
  'authorization, content-type, mcp-session-id, mcp-protocol-version, x-aedifex-mcp-token'
...
function createHttpGuard(options: {
  ...
  return (req, res) => {
    ...
    if (options.authToken) {
      const supplied = bearerToken(req) ?? headerValue(req.headers['x-aedifex-mcp-token'])
