...
The branch is now local-first: scenes persist to `~/.aedifex/data/aedifex.db` through SQLite, using `bun:sqlite` in the MCP CLI and `node:sqlite` when the Next.js editor server imports the storage package. The earlier Supabase adapter, SQL migrations, and committed `test-reports/` artifacts have been removed.
...
```json
{
  "mcpServers": {
    "pascal": {
      ...
      "env": {
        "AEDIFEX_DATA_DIR": "/Users/you/.aedifex/data"
      }
    }
  }
}
...
codex mcp add pascal-dev \
  --env AEDIFEX_DATA_DIR="$HOME/.aedifex/data" \
