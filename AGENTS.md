...
packages/
  ...
  editor/          # Editor components (tools, panels, sidebar)
...
Next.js 16 app. Composes `@aedifex/viewer` and `@aedifex/core` into a full editing experience.
...
- **Viewer isolation** — `@aedifex/viewer` must never import from `apps/editor`. Editor-specific behaviour (tools, systems, selection) is injected as children or props.
